...
class CalendarStripWidget extends StatefulWidget {
  ...
  final Icon? leftIcon; // Left icon
  final bool
      isChangeDateSelected; // Is change date selected from calendar default or other
...
class CalendarStripState extends State<CalendarStripWidget>
  ...
  static const sizeSelectedDay = 32.0; // Size selected day

  DateTime currentDate = DateTime.utc(
      DateTime.now().year, DateTime.now().month, DateTime.now().day);
  DateTime? selectedDate; // Selected date
  String? monthLabel; // Month label
  bool inBetweenMonths = false; // Is between months
  DateTime? rowStartingDate; // Row starting date
  double opacity = 0.0; // Opaciy
  DateTime? lastDayOfMonth; // Last day of month
  TextStyle monthLabelStyle = CommonStyle.textLargeBold(); // Month label style
  TextStyle selectedDateStyle =
      CommonStyle.textBold(color: Colors.primary); // Selected date style
  bool? isOnEndingWeek = false, isOnStartingWeek = false; // Is on ending week
  bool doesDateRangeExists = false; // Does date range exits
  ...
  List<String> dayLabels = Globals.language == LanguageCode.VI
  ...
  buildDateRow() {
    ...
    return Column(children: [
      ...
      Container(
          margin: EdgeInsets.only(top: Constants.margin12),
  ...
  build(BuildContext context) {
    return Container(
      height: nullOrDefault(widget.containerHeight, 100.0),
      child: buildDateRow(),
